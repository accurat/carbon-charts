		...
		this.services.events.addEventListener("show-tooltip", e => {
			...
			if (
				...
				(e.detail.type === TooltipTypes.GRIDLINE &&
					Tools.getProperty(this.model.getOptions(), "tooltip", "gridline", "enabled"))
			) {
				...
				const chartWidth =
					...
					Tools.getProperty(this.model.getOptions(), "tooltip", "title", "width");

				this.tooltip.style("max-width", chartWidth);

				tooltipTextContainer.html(
					this.getTooltipHTML(e.detail.hoveredElement, TooltipTypes.TITLE)
				);
			...
			points
				.map(datapoint => {
					...
					const formattedValue = Tools.getProperty(
						...
						? this.model.getOptions().tooltip.valueFormatter(datapointValue)
						: datapointValue.toLocaleString("en");
