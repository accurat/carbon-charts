...
export class Pie extends Component {
	...
	render(animate = true) {
		...
		const pieLayoutData = pieLayout(dataList).sort((a: any, b: any) => a.index - b.index);

		// Update data on all slices
		const slicesGroup = DOMUtils.appendOrSelect(svg, "g.slices").attr("role", Roles.GROUP);
		const paths = slicesGroup.selectAll("path.slice").data(pieLayoutData, d => d.data.label);
		...
		enteringPaths
			...
			.attr(
				"aria-label",
				d => `${d.value}, ${Tools.convertValueToPercentage(d.data.value, dataList) + "%"}`
			)
			// Tween
	...
	renderCallouts(calloutData: any[]) {
		const svg = DOMUtils.appendOrSelect(this.getContainerSVG(), "g.callouts").attr(
			"role",
			Roles.GROUP
		);
		const options = this.model.getOptions();
