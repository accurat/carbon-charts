	...
	getStackData() {
		...
		if (timeSeries) {
			...
			stackDataArray = allDates.map((date, i) => {
				...
				displayData.datasets.forEach(dataset => {
					const correspondingDatum = dataset.data.find(
						datum => Number(datum.date) === Number(date)
					);
			...
			stackDataArray = displayData.labels.map((label, i) => {
				...
				displayData.datasets.forEach(dataset => {
					correspondingData[dataset.label] = !isNaN(dataset.data[i])
						? dataset.data[i]
		...
		bars.enter()
			...
			.attr("height", (d, i) => {
				...
				if (yScale.scaleType === ScaleTypes.LABELS) {
					height = Math.abs(
						yScale.scale.range()[0] -
							this.services.cartesianScales.getRangeValue(d.label, i)
					);
				} else {
					height =
						this.services.cartesianScales.getRangeValue(d[0]) -
						this.services.cartesianScales.getRangeValue(d[1]);
				}
	...
	addEventListeners() {
		...
		this.parent
			...
			.on("mousemove", function(datum) {
				...
				const activePoints = Object.keys(stackedData).map(key => ({
					datasetLabel: key,
